...
1、打开配置文件里的 `installation.yaml`，将 `installation_id` 后面改为`Mac`（支持自定义）。
2、复制下面路径代码粘贴进来，代码中的 `admin` 替换为你的电脑用户名。
...
1. 打开配置文件里的 `installation.yaml`，将内容修改为 iCloud 同步文件里  `installation.yaml` 的一至。
